            ...
            <div className="grid md:grid-cols-2 gap-6 mb-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none disabled:bg-slate-50"
                  required
                />
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-8 shadow-lg">
            ...
            {metodoPago === 'tarjeta' && (
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <input type="text" value={datosTarjeta.numero} onChange={(e) => { const v = e.target.value.replace(/\s/g, '').replace(/\D/g, ''); setDatosTarjeta({ ...datosTarjeta, numero: v.match(/.{1,4}/g)?.join(' ') || v })}} placeholder="1234 5678 9012 3456" maxLength={19} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Titular de la Tarjeta</label>
                  <input type="text" value={datosTarjeta.titular} onChange={(e) => setDatosTarjeta({ ...datosTarjeta, titular: e.target.value })} placeholder="Juan Pérez" className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Fecha de Expiración</label>
                    <input type="text" value={datosTarjeta.expiracion} onChange={(e) => { let v = e.target.value.replace(/\D/g, ''); if (v.length >= 2) v = v.slice(0, 2) + '/' + v.slice(2, 4); setDatosTarjeta({ ...datosTarjeta, expiracion: v })}} placeholder="MM/AA" maxLength={5} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">CVV</label>
                    <input type="text" value={datosTarjeta.cvv} onChange={(e) => setDatosTarjeta({ ...datosTarjeta, cvv: e.target.value.replace(/\D/g, '') })} placeholder="123" maxLength={3} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber-500 outline-none" />
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="space-y-1 text-sm text-slate-600">
                ...
                <p className="text-lg font-bold text-amber-600 mt-2">Total: ${total.toLocaleString('es-ES')}</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-8 shadow-lg text-center">
            ...
            <div className="bg-slate-50 rounded-lg p-6 mb-6 text-left border border-slate-200">
              ...
              <div className="space-y-2 text-sm text-slate-700">
                ...
                <p className="text-lg font-bold text-amber-600 mt-3">Total pagado: ${total.toLocaleString('es-ES')}</p>
              </div>
            </div>
            <button onClick={() => navigate('/usuario/dashboard')} className="px-8 py-3 bg-amber-600 hover:bg-amber-700 text-white font-semibold rounded-lg transition-all">
              Volver al Dashboard
            </button>
          </div>
        )}
